...
Choose **Orbit** or **Fly** beside the Frame button, or press `V` to toggle modes. In Orbit, right-drag pans, middle-drag orbits, and the wheel zooms. In Fly, hold RMB to capture the pointer, look with the mouse, move with `W`/`S`, strafe with `A`/`D`, and move along world Z with `Q`/`E`; hold Shift for a 4× boost and use the wheel to adjust flight speed. Release RMB or press Escape to release the pointer. Press `F` to frame the mesh and return to Orbit.
