...
- `src/mesh.rs` owns validated mesh data, adjacency, normals, and spatial queries; `src/sculpt.rs` applies and validates fixed-topology brush operations; `src/stroke.rs` retains captured input state while scheduling distance- and time-based brush dabs.
